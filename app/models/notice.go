...
type Notice struct {
	Id         int       `orm:"column(id);auto" description:"通知ID"`
	Title      string    `orm:"column(title);size(150)" description:"通知标题"`
	Content    string    `orm:"column(content)" description:"通知内容"`
	Source     int       `orm:"column(source)" description:"来源：1内部通知 2外部通知"`
	IsTop      int       `orm:"column(is_top)" description:"是否置顶：1是 2否"`
	Browse     int       `orm:"column(browse)" description:"阅读量"`
	Status     int       `orm:"column(status)" description:"状态：1已发布 2待发布"`
